    ...
    <div class="">
        ...
        <div class="row">
            <div class="col-md-12 col-sm-12 ">
                <div class="x_panel">
                    <div class="x_title">
                        <h2>Listagem |<small>Armadores</small></h2>
                        <div class="clearfix"></div>
                                    ...
                                    <table id="datatable-responsive"
                                        ...
                                        <thead>
                                            <tr>
                                                ...
                                                <th>Nº BI ou NIF</th>
                                                <th>Telefone</th>
                                                <th>Tipo de armador</th>
                                                <th class="option-title">Opção</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            @forelse($fishers as $key => $fisher)
                                                <tr>
                                                    <td>
                                                        <div class="fisher-cover-img">
                                                    ...
                                                    <td>{{ $fisher->bi }}</td>
                                                    <td>{{ $fisher->phone }}</td>
                                                    <td>{{ $fisher->fisher_type }}</td>
                                                    <td class="option-data">
                                                        <a href="#" data-toggle="modal" data-toggle="tooltip" data-placement="top" title="Visualização" data-target="#{{$fisher->fisher_type}}{{$fisher->id}}"
                                                            class="btn btn-primary btn-round show-armador"><i
                                                                class="fa fa-eye" aria-hidden="true"></i> <span
                                                                class="option-title"></span></a>
                                                        @can('just-admin-or-manager')
                                                            <a href="{{ route('fisher.edit.form', encrypt($fisher->id) ) }}" data-toggle="tooltip" data-placement="top" title="Actualização"
                                                                class="btn btn-info btn-round"><i class="fa fa-refresh"
                                                                    aria-hidden="true"></i> <span
                                                                    class="option-title"></span></a>
                                                            @can('only-admin')
                                                                <form
                                                                    action="{{ route('fisher.remove') }}"
                                                                    method="POST" style="display: inline">
                                                                    {{ csrf_field() }}
                                                                    <input type="hidden" name="element"
                                                                        value="{{ $fisher->id }}">
                                                                    <button class="btn btn-danger btn-round" data-toggle="tooltip" data-placement="top" title="Remoção"
        ...
        document.querySelector('#tablePDF').addEventListener('click', () => {
            ...
            doc.autoTable({
                ...
                didDrawPage: function (data) {
                    ...
                    var tableTitle = 'Lista de Armadores'
            ...
            doc.save("lista-de-armadores.pdf");
        });
    </script>
